-- The live prevent_booking_conflicts trigger was missing the early-exit guard
-- for fitness_studio class bookings (service_ids = '{}'). Without it, the
-- PRIVATE branch ran with service_ids[1] = NULL → primary_max_capacity = NULL
-- → COALESCE(NULL,1) = 1 → blocked every second approval for the same slot.
-- This migration adds the guard while preserving all existing is_parallel logic.
...
BEGIN
  ...
  -- Class bookings (fitness_studio) store service_ids = '{}'. Their capacity
  -- is enforced at the application layer via class_schedule.max_capacity.
  -- Skip all conflict checks for them.
  ...
  -- CLAUSE 1: reject if any overlapping booking is for a DIFFERENT primary
  -- service. Exception: both sides are parallel → allowed to overlap.
  IF EXISTS (
    SELECT 1
    FROM public.bookings b
    LEFT JOIN LATERAL (
  ...
  -- SAME-USER guard: one account may hold only ONE overlapping booking with
  -- this provider on this date. Walk-ins (user_id IS NULL) are excluded.
  ...
  -- CLAUSE 2: count overlapping bookings for the SAME primary service;
  -- reject once the pool is at capacity.
  SELECT COUNT(*)
  INTO current_count
  FROM public.bookings b
  LEFT JOIN LATERAL (
